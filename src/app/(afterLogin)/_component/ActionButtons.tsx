...
export default function ActionButtons({ white, post }: Props) {
      ...
      queryKeys.forEach((queryKey) => {
        if (queryKey[0] === "posts") {
          ...
          if (value && "pages" in value) {
            //console.log("name" in person); true or false를 리턴
            const obj = value.pages.flat().find((v) => v.postId === postId); //2차원배열을 1차원으로 플랫시킴
            if (obj) {
              //존재하는지
            ...
            if (value.postId === postId) {
              const shallow = {
                ...value,
                Hearts: [{ userId: session?.user?.email as string }], //아이디 추가
                _count: {
                  ...value._count,
        ...
        if (queryKey[0] === "posts") {
          ...
          if (value && "pages" in value) {
            ...
            if (value.postId === postId) {
              const shallow = {
                ...value,
                Reposts: [{ userId: session?.user?.email as string }], //아이디 추가
                _count: {
                  ...value._count,
  ...
  const onClickComment: MouseEventHandler<HTMLButtonElement> = (e) => {
    ...
    modalStore.setMode("comment"); //zustand
